...
extension AppDelegate {
  ...
  func initAllModules(_ appKey: String, extra: [String: String] = [String: String](), callback: @escaping (Int, String?) -> Void) {
    var listenTogetherInit = false
    var orderSongInit = false
    
    func checkCallback() {
      if listenTogetherInit,
         orderSongInit{
        callback(0, nil)
      }
    }
    initListenTogether(appKey, extras: extra) { code, msg, _ in
      if code != 0 {
        callback(code, msg)
      } else {
        initQueue.async {
          listenTogetherInit = true
          checkCallback()
        }
      }
    }
  ...
  func initListenTogether(_ appKey: String, extras: [String: String] = [String: String](), callback: @escaping (Int, String?, Any?) -> Void) {
...
extension AppDelegate {
  ...
  public func loginRoom(shouldInit: Bool = false, callback:((Bool)->Void)? = nil) {
    ...
    func login() {
      NEListenTogetherUIManager.sharedInstance().login(withAccount: userUuid, token: userToken, nickname: userName) { code, msg, _ in
        if code == 0 {
...
extension AppDelegate: NEListenTogetherUIDelegate {
  ...
  func leaveOtherRoom(completion: (() -> Void)? = nil) {
    NEVoiceRoomKit.getInstance().leaveRoom { _,_,_ in
      DispatchQueue.main.async {
      }
      completion?()
    }
  ...
  func onListenTogetherJoinRoom() {
    
  }
  
  func onListenTogetherLeaveRoom() {
    
  }
  
  func onListenTogetherClientEvent(_ event: NEListenTogetherClientEvent) {
    switch event {
