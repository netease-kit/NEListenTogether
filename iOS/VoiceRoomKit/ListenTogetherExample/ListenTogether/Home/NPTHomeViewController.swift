...
extension NPTHomeViewController: UITableViewDelegate {
  func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
    ...
    if indexPath.section == 0 {
      let vc = NEListenTogetherRoomListViewController()
...
extension NPTHomeViewController: UITableViewDataSource {
  ...
  func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    if let cell = tableView.dequeueReusableCell(withIdentifier: "NPTHomeCellIdentifier") as? NPTHomeCell {
      ...
      if indexPath.section == 0 {
        cell.backgroundImage.image = UIImage(named: "home_blue")
        cell.lottieView.animation = LottieAnimation.named("listenTogether")
        cell.typeLabel.text = "Listen_Together".localized
        cell.descriptionLabel.text = "Listen_Together_Description".localized
