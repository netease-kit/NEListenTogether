...
@property(nonatomic, strong) NEVoiceRoomSeatItem *seatItem;
@end
...
- (void)_loadData:(NEVoiceRoomSeatItem *)data indexPath:(NSIndexPath *)indexPath {
  self.seatItem = data;
  _nameLabel.text =
...
+ (NEListenTogetherUIConnectListCell *)cellWithTableView:(UITableView *)tableView
  ...
  if ([datas count] > indexPath.row) {
    NEVoiceRoomSeatItem *data = [datas objectAtIndex:indexPath.row];
