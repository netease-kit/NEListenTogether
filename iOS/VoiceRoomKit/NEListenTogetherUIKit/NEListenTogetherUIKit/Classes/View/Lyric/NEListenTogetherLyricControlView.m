...
- (NEListenTogetherControlButton *)pauseBtn {
  if (!_pauseBtn) {
    ...
    [_pauseBtn addAction:^(NEListenTogetherControlButton *button) {
      if (button.selected) {
        if ([weakSelf.delegate respondsToSelector:@selector(resumeSongWithView:)]) {
          [weakSelf.delegate resumeSongWithView:weakSelf];
        }
      } else {
        if ([weakSelf.delegate respondsToSelector:@selector(pauseSongWithView:)]) {
          [weakSelf.delegate pauseSongWithView:weakSelf];
        }
      }
    }];
  }
...
- (NEListenTogetherControlButton *)nextBtn {
  if (!_nextBtn) {
    ...
    __weak typeof(self) weakSelf = self;
    [_nextBtn addAction:^(NEListenTogetherControlButton *button) {
      if ([weakSelf.delegate respondsToSelector:@selector(nextSongWithView:)]) {
        [weakSelf.delegate nextSongWithView:weakSelf];
      }
    }];
  }
  return _nextBtn;
