...
- (NEListenTogetherLyricView *)lyricView {
  if (!_lyricView) {
    ...
    _lyricView.timeForCurrent = ^NSInteger {
      if ([weakSelf.delegate respondsToSelector:@selector(onLyricTime)]) {
        return [weakSelf.delegate onLyricTime];
      } else {
        return 0;
      }
    };
    _lyricView.seek = ^(NSInteger seek) {
      if ([weakSelf.delegate respondsToSelector:@selector(onLyricSeek:)]) {
        [weakSelf.delegate onLyricSeek:seek];
      }
    };
  }
  return _lyricView;
