...
- (void)setContent:(NSString *)content lyricType:(NELyricType)type {
  ...
  [self
      preloadLyricCompleteBlock:^{
        ...
        if (strongSelf) {
          [strongSelf.lyricView loadWithLyricModel:strongSelf.model];
        }
      }
...
- (void)setPath:(NSString *)path lyricType:(NELyricType)type {
  ...
  [self
      preloadLyricCompleteBlock:^{
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (strongSelf) {
          [weakSelf.lyricView loadWithLyricModel:weakSelf.model];
        }
      }
                      lyricList:(NSArray<NELyricLine *> *)self.model.lineModels];
}
...
- (void)preloadLyricCompleteBlock:(void (^)(void))completeBlock
  ...
  [lyricList bk_each:^(NELyricLine *obj) {
    ...
    obj.layoutInfo.lyricUpdateHandler = ^(YYLabel *_Nonnull label, NSString *_Nonnull lyricText) {
      if (lyricText.length > 0) {
        label.numberOfLines = 0;
        if (!weakObj.layoutInfo.lyricTextAttrStr) {
          ...
          weakObj.layoutInfo.lyricTextAttrStr = attr;
        }
        label.attributedText = weakObj.layoutInfo.lyricTextAttrStr;
      }
    };
...
- (NELyricView *)lyricView {
  if (!_lyricView) {
    ...
    _lyricView.timeForCurrent = ^NSInteger {
      return weakSelf.timeForCurrent();
    };
  }
  return _lyricView;
}
...
- (UIButton *)seekBtn {
  if (!_seekBtn) {
    ...
    __weak typeof(self) weakSelf = self;
    [_seekBtn bk_whenTapped:^{
      if (weakSelf.model.lineModels.count > 0) {
        NELyricLine *line = weakSelf.model.lineModels.firstObject;
        NSInteger startTime = line.startTime;
        if (line.words.count) {
          startTime = line.words.firstObject.startTime;
        }
        weakSelf.seek(startTime - 5 * 1000);
