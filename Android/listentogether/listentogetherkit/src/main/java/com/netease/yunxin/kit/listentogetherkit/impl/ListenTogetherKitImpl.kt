...
import com.netease.yunxin.kit.common.network.ServiceCreator.Companion.ACCEPT_LANGUAGE_KEY
...
import com.netease.yunxin.kit.listentogetherkit.api.NEListenTogetherAuthListener
import com.netease.yunxin.kit.listentogetherkit.api.NEListenTogetherCallback
import com.netease.yunxin.kit.listentogetherkit.api.NEListenTogetherErrorCode
...
import com.netease.yunxin.kit.listentogetherkit.api.model.NEListenTogetherLanguage
...
internal class ListenTogetherKitImpl : NEListenTogetherKit, CoroutineRunner() {
    ...
    private var createVoiceRoomInfo: ListenTogetherRoomInfo? = null
    private var joinedVoiceRoomInfo: ListenTogetherRoomInfo? = null
    private val myRoomService = ListenTogetherService()
    private val authListeners: ArrayList<NEListenTogetherAuthListener> by lazy { ArrayList() }
    private lateinit var context: Context
    private var hasLogin: Boolean = false
    private val listeners = ArrayList<NEListenTogetherRoomListener>()
    companion object {
        private const val tag = "NEListenTogetherKit"
        ...
        NERoomKit.getInstance().getService(NEAuthService::class.java).addAuthListener(object :
            ...
            override fun onAuthEvent(evt: NEAuthEvent) {
                ...
                authListeners.forEach {
                    it.onListenTogetherAuthEvent(
                        NEListenTogetherAuthEvent.fromValue(evt.name.uppercase(Locale.getDefault()))
                    )
                }
            }
        })
        ...
        launch {
            voiceRoomHttpService.httpErrorEvents.collect { evt ->
                if (evt.code == NEErrorCode.UNAUTHORIZED ||
                    ...
                    authListeners.forEach {
                        it.onListenTogetherAuthEvent(NEListenTogetherAuthEvent.UNAUTHORIZED)
                    }
                } else if (evt.code == NEErrorCode.TOKEN_EXPIRED) {
                    authListeners.forEach {
                        it.onListenTogetherAuthEvent(
                            NEListenTogetherAuthEvent.ACCOUNT_TOKEN_ERROR
                        )
                    }
                }
            }
    ...
    private fun initRoomServiceListener() {
        myRoomService.addListener(object : NEListenTogetherRoomListener {
            override fun onMemberJoinRoom(members: List<NEListenTogetherRoomMember>) {
                listeners.forEach {
                    it.onMemberJoinRoom(members)
                }
            }

            override fun onMemberLeaveRoom(members: List<NEListenTogetherRoomMember>) {
                listeners.forEach {
                    it.onMemberLeaveRoom(members)
                }
            }

            override fun onMemberJoinChatroom(members: List<NEListenTogetherRoomMember>) {
                listeners.forEach {
                    it.onMemberJoinChatroom(members)
                }
            }

            override fun onMemberLeaveChatroom(members: List<NEListenTogetherRoomMember>) {
                listeners.forEach {
            ...
            override fun onMemberAudioMuteChanged(member: NEListenTogetherRoomMember, mute: Boolean, operateBy: NEListenTogetherRoomMember?) {
                listeners.forEach {
    ...
    fun initVoiceRoomHttpService() {
        voiceRoomHttpService.initialize(context)
        val localLanguage = Locale.getDefault().language
        voiceRoomHttpService.addHeader(ACCEPT_LANGUAGE_KEY, localLanguage)
    }
    ...
    override fun login(account: String, token: String, callback: NEListenTogetherCallback<Unit>?) {
        ...
        if (hasLogin) {
            ...
            ListenTogetherLog.i(tag, "login but isLoggedIn = true")
    ...
    override fun addAuthListener(listener: NEListenTogetherAuthListener) {
        ListenTogetherLog.logApi("addAuthListener: listener=$listener")
        authListeners.add(listener)
    }

    override fun removeAuthListener(listener: NEListenTogetherAuthListener) {
    ...
    override fun getCurrentRoomInfo(): NEListenTogetherRoomInfo? {
    ...
    override fun addRoomListener(listener: NEListenTogetherRoomListener) {
        ListenTogetherLog.logApi("addRoomListener: listener=$listener")
        listeners.add(listener)
    }

    override fun removeRoomListener(listener: NEListenTogetherRoomListener) {
        ListenTogetherLog.logApi("removeRoomListener: listener=$listener")
    ...
    override fun switchLanguage(language: NEListenTogetherLanguage): Int {
        when (language) {
            NEListenTogetherLanguage.AUTOMATIC -> {
                val localLanguage = Locale.getDefault().language
            ...
            NEListenTogetherLanguage.CHINESE -> {
            ...
            NEListenTogetherLanguage.ENGLISH -> {
            ...
            else -> return NEListenTogetherErrorCode.FAILURE
        }
    }
}
