...
import com.netease.yunxin.app.listentogether.adapter.*;
import com.netease.yunxin.app.listentogether.dialog.*;
import com.netease.yunxin.kit.entertainment.common.utils.Utils;
import com.netease.yunxin.kit.listentogether.*;
import com.netease.yunxin.kit.listentogetherkit.impl.utils.*;
import java.util.*;
...
public class GiftDialog extends BottomBaseDialog {
  ...
  protected void renderTopView(FrameLayout parent) {
    ...
    titleView.setText(R.string.listen_send_gift);
  ...
  protected void renderBottomView(FrameLayout parent) {
    View bottomView =
        LayoutInflater.from(getContext()).inflate(R.layout.listen_view_dialog_bottom_gift, parent);
    // 礼物列表初始化
  ...
  private static class InnerAdapter extends LiveBaseAdapter<GiftInfo> {
    ...
    protected int getLayoutId(int viewType) {
      return R.layout.listen_view_item_dialog_gift;
    }
    ...
    protected void onBindViewHolder(
      ...
      if (itemData == focusedInfo) {
        border.setBackgroundResource(R.drawable.listen_layer_dialog_gift_chosen_bg);
      } else {
    ...
    private String formatValue(long value) {
      return "(" + value + Utils.getApp().getString(R.string.listen_coin) + ")";
    }
  }
}
