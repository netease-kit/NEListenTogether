...
public class ListenTogetherService {
  ...
  private final NEMessageChannelListener messageChannelListener =
      message -> {
        if (message.getCommandId() == ListenTogetherCmd.ASK_PLAYING_POSITION_CMD) {
          replyPlayingPosition(message);
        } else if (message.getCommandId() == ListenTogetherCmd.SYNC_PLAYING_POSITION_CMD) {
        ...
        } else if (message.getCommandId() == ListenTogetherCmd.NOTIFY_OTHER_MY_SONG_IS_READY_CMD) {
          JSONObject jsonObject = null;
          try {
            jsonObject = new JSONObject(message.getData());
            String songId = jsonObject.optString(ListenTogetherConstant.SONG_ID);
  ...
  private void replyPlayingPosition(NECustomMessage customMessage) {
    ...
    sendCustomMessage(
        ...
        ListenTogetherCmd.SYNC_PLAYING_POSITION_CMD,
        jsonObject.toString(),
        (code, message, unit) -> {
          ALog.i(
  ...
  public void notifyOtherMySongIsDownloading(Song song, boolean isDownloading) {
    ...
    sendCustomMessage(
        roomUuid,
        getOtherUserUuid(),
        ListenTogetherCmd.NOTIFY_OTHER_MY_SONG_IS_READY_CMD,
        jsonObject.toString(),
        (code, message, unit) -> {
  ...
  public void queryCurrentSongPlayPosition() {
    ...
    sendCustomMessage(
        roomUuid,
        getAnotherUserUuid(),
        ListenTogetherCmd.ASK_PLAYING_POSITION_CMD,
        jsonObject.toString(),
        (code, message, unit) -> {
          ALog.i(
